public class SegmentTree<E> {
    ...
    // 在以treeIndex为根的线段树中[l...r]的范围中，搜索区间为[queryL. queryR]的值
    private E query(int treeIndex, int l, int r, int queryL, int queryR) {
        ...
        int mid = l + (r -l)/2;
        int leftChildIndex = leftChild(treeIndex);
        int rightChildIndex = leftChild(treeIndex);
        if (queryR <= mid){
        ...
        return (E) merger.merge(leftResult, rightResult);
    }
    ...
    // 返回完全二叉树的数组表示中，一个索引为index的元素的左孩子节点的索引
    private int leftChild(int index) {
        return 2 * index + 1;
    }

    // 返回完全二叉树的数组表示中，一个索引为index的元素的右孩子节点的索引
